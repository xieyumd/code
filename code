...
# 邮件通知进度用到的函数
# ================================================================================
my_sender = '15026975166@163.com'  # 发件人邮箱账号
my_user = 'subxy@hotmail.com'  # 收件人邮箱账号


def email_progress(myMessage):
    try:
        msg = MIMEText(myMessage, "plain", "utf-8")
        msg['From'] = formataddr(['xieyu', my_sender])  # 括号里的对应发件人邮箱昵称（str）、发件人邮箱账号
        msg['To'] = formataddr(['xieyu', my_user])  # 括号里的对应收件人邮箱昵称（str）、收件人邮箱账号
        msg['Subject'] = "你家的小虫喊你回家吃饭"  # 邮件的主题，也可以说是标题
        server = smtplib.SMTP_SSL("smtp.163.com", 25)  # 发件人邮箱中的SMTP服务器，端口是25,SSL加密则根据各网站服务器定义的写
        server.login(my_sender, "888xieyu")  # 括号中对应的是发件人邮箱账号、邮箱密码
